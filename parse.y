...
%start cmd_line
%token <string> EXIT WAIT KILL DECLARE CD PUSHD POPD HELP PIPE INPUT_REDIR OUTPUT_REDIR STRING NL BACKGROUND
...
cmd_line    :
  ...
  | WAIT           { current_builtin = BUILTIN_WAIT;    builtin_arg = NULL; }
  ...
  | DECLARE        { current_builtin = BUILTIN_DECLARE; builtin_arg = NULL; }
  | DECLARE STRING { current_builtin = BUILTIN_DECLARE; builtin_arg = strdup($2); }
  | CD             { current_builtin = BUILTIN_CD;      builtin_arg = NULL; }
  | CD STRING      { current_builtin = BUILTIN_CD;      builtin_arg = strdup($2); }
  | PUSHD STRING   { current_builtin = BUILTIN_PUSHD;   builtin_arg = strdup($2); }
  | POPD           { current_builtin = BUILTIN_POPD;    builtin_arg = NULL; }
  | HELP           { current_builtin = BUILTIN_HELP;    builtin_arg = NULL; }
  | HELP STRING    { current_builtin = BUILTIN_HELP;    builtin_arg = strdup($2); }
  | pipeline background
  ;
